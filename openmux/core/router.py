...
class Router:
    ...
    def __init__(self, timeout: float = 30.0, max_retries: int = 3, max_concurrency: Optional[int] = None):
        """Initialize the router.
        
        Args:
            timeout: Request timeout in seconds
            max_retries: Maximum number of retry attempts
            max_concurrency: Optional maximum number of concurrent provider calls.
                If provided and > 0, route_multiple will limit simultaneous
                provider calls using an asyncio.BoundedSemaphore.
        """
        ...
        logger.info(f"Router initialized (timeout={timeout}s, retries={max_retries}, max_concurrency={max_concurrency})")
    ...
    async def route_multiple(
        ...
        **kwargs
        ...
        # Create a task for each provider so they all run in parallel. If a
        # semaphore is configured, each task will acquire it before calling
        # route_single which limits simultaneous provider calls.
        tasks = []
        for provider in providers:
            async def _invoke(p=provider):
                if self._semaphore:
                    async with self._semaphore:
                        return await self.route_single(p, query, **kwargs)
                return await self.route_single(p, query, **kwargs)

            tasks.append(asyncio.create_task(_invoke()))
